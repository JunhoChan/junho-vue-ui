<template>
  <div
    ...
    <div class="navMenu-link">
      <img alt="" src="@app/assets/svg/theme.svg" class="navMenu-link-icon" />
...
import { routerData } from '@app/router/routes'
  ...
  methods: {
    ...
    handleResize(isRegister) {
      const handleEvnet = Utils.debounce((event) => {
        this.$store.commit('config/setMinScreen', event.target.innerWidth <= 1000)
